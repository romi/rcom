...
serial_t *new_serial(const char *device, int speed)
{
        ...
        if (s->device == NULL || s->in == NULL || s->out == NULL) {
                delete_serial(s);
                return NULL;
        }
...
void delete_serial(serial_t *s)
{
        if (s) {
                serial_lock(s);
                if (s->device) mem_free(s->device);
                if (s->in) delete_membuf(s->in);
                if (s->out) delete_membuf(s->out);
                close(s->fd);
                s->fd = -1;
                serial_unlock(s);
...
const char *serial_readline(serial_t *s)
{
        ...
        while (1) {
		  ...
		  return NULL;
                        ...
                        if (serial_peek(s) == '\n')
                                c = serial_get(s);
                        r = membuf_append_zero(s->in);
                        if (r != 0) return NULL;
                        return membuf_data(s->in);
...
const char *serial_command_send(serial_t *s, const char *cmd)
{
	...
	//log_debug("serial_command_send: reply %s", reply);
